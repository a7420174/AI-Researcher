...
def with_env(env: BrowserEnv):
    ...
    def decorator(func):
        ...
        wrapped.__signature__ = signature(func).replace(
            parameters=[
                p for p in signature(func).parameters.values() if p.name != "env"
            ]
        )
        return wrapped
...
def wrap_return_value(web_obs: WebObservation, action_description: str = ""):
    ...
    if web_obs.error:
        error_prefix = get_error_prefix(
            web_obs.last_browser_action, web_obs.last_browser_action_error
        )
    cur_url = web_obs.url
    try:
...
@register_tool("click")
def click(
    env: BrowserEnv, bid: str, button: Literal["left", "middle", "right"] = "left"
):
    """
    Clicks the mouse on the target with the given element bid.
    Args:
...
def visit_url(env: BrowserEnv, url: str):
    ...
    try:
        if url.startswith(("https://", "http://", "file://", "about:")):
            ...
            action_str = (
                f"_visit_page('https://www.google.com.sg/search?q={query}&hl=en&gl=US')"
            )
            obs = env.step(action_str)
            web_obs = to_web_obs(obs)
        else:
...
def ddg_search(query: str, max_results: int = 10):
    """
    Performs a web search on DuckDuckGo (no rate limits, no CAPTCHA).
...
def truncate_by_tokens(
    ...
    from research_agent.inno.tools.files import create_file, create_directory
    ...
    output_path = (
        f"{env.docker_workplace}/web_page_markdown_output/full_output_{timestamp}.md"
    )
...
def flatten_google_scholar_results(dom_snapshot, extra_properties: dict = None) -> list:
    ...
    if (
        not dom_snapshot
        or "documents" not in dom_snapshot
        or not dom_snapshot["documents"]
    ):
        return []
    ...
    def parse_document(document_idx) -> list:
        ...
        def find_node_by_class(root_idx, target_class):
            ...
            if nodes["nodeType"][root_idx] == 1:  # 元素节点
                ...
                for i in range(0, len(attrs), 2):
                    if to_string(attrs[i]) == "class" and target_class in to_string(
                        attrs[i + 1]
                    ):
                        return root_idx
            ...
            if nodes["nodeType"][node_idx] == 1:  # 元素节点
                ...
                for i in range(0, len(attrs), 2):
                    ...
                    if (
                        attr_name == "class"
                        and attr_value
                        and "gs_r gs_or gs_scl" in attr_value
                    ):
                        is_paper_block = True
                        break
    ...
    try:
        ...
        raise ValueError(
            f"Error encountered when extracting the paper information from the Google Scholar search results: {e}"
        )
    ...
    for result in result_list:
        if result["pdf_link"] == "":
            ...
            if "link.springer.com" in result["title_link"]:
                ...
                result["pdf_link"] = (
                    f"https://link-springer-com.eproxy.lib.hku.hk/content/pdf/{doi}.pdf"
                )
                ...
                result["pdf_link"] = (
                    f"https://dl-acm-org.eproxy.lib.hku.hk/doi/pdf/{doi}"
                )
                ...
                result["pdf_link"] = (
                    f"https://ieeexplore-ieee-org.eproxy.lib.hku.hk/stamp/stamp.jsp?tp=&arnumber={doi}"
                )
                ...
                doi = result["title_link"].split("article/")[1]
                result["pdf_link"] = (
                    f"https://www-nature-com.eproxy.lib.hku.hk/articles/{doi}.pdf"
                )
                ...
                doi = result["title_link"].split("abs/")[1]
                result["pdf_link"] = (
                    f"https://www-science-org.eproxy.lib.hku.hk/doi/pdf/{doi}?download=true"
                )
            else:
                result_list.remove(result)
    for result in result_list:
        assert result["pdf_link"] != "", (
            f"The pdf link of the paper {result['title']} is not found"
        )
    return result_list
...
def wrap_google_scholar_search(web_obs: WebObservation, action_description: str = ""):
    ...
    try:
        ...
        cur_axtree_txt = flatten_google_scholar_results(
            web_obs.dom_object, extra_properties=web_obs.extra_element_properties
        )
...
def google_scholar_search(
    context_variables, env: BrowserEnv, query: str, page: int = 0
):
    """
...
if __name__ == "__main__":
    ...
    env = BrowserEnv(
        ...
        local_root="/home/tjb/llm/agent/Inno-agent",
        workplace_name="workplace_paper_eval",
    )
