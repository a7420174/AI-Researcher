...
class SectionComposer(ABC):
    ...
    def load_checkpoint(self, target_paper: str, step: str) -> Optional[Dict]:
        ...
        checkpoint_file = os.path.join(
            self.get_checkpoint_path(target_paper), f"{step}.json"
        )
    ...
    def get_random_template(self) -> str:
        ...
        try:
            with open(
                os.path.join(template_dir, selected_template), "r", encoding="utf-8"
            ) as f:
                return f.read()
    ...
    async def fuse_subsections(
        self, structure: str, subsection_contents: Dict[str, str]
    ) -> str:
